...
let data = null, selected = new Set(), oldSelected = new Set(), offset = 0, visible = [], refreshing = false, initialized = false, pendingConfirm = null, busy = false;
...
function syncSelection() {
  ...
  const eligible = visible.filter(r => r.state === 'ready');
...
function render() {
  ...
  for (const id of selected) if (!items.some(r => r.id === id && r.state === 'ready')) selected.delete(id);
  ...
  if (!$('matches').contains(document.activeElement)) $('matches').innerHTML = visible.map(r => `<article class="return-row"><div class="return-file"><input class="job-checkbox" type="checkbox" data-select="move" value="${esc(r.id)}" aria-label="Vælg ${esc(r.input)}" ${r.state === 'ready' ? '' : 'disabled'}><div><strong>${esc(r.input.split('/').pop())}</strong><span class="badge ${r.state === 'ready' || r.state === 'done' ? 'completed' : r.state === 'attention' || r.state === 'failed' ? 'failed' : 'queued'}">${esc(names[r.state] || r.state)}</span><p class="hint">${esc(r.error)}</p><div class="return-paths"><span>Fra</span><code>${esc(r.input)}</code><span>Til</span><code>${esc(r.details.target || r.original || 'Afventer match')}</code></div><p class="hint">${r.mode === 'reelshrink' ? 'Reelshrink-job' : 'Navnesammenligning'} · ${bytes(r.details.inputBytes)}${r.details.originalBytes ? ' · original ' + bytes(r.details.originalBytes) : ''}</p>${r.state === 'attention' && r.details.backup ? `<p class="hint">Original/backup: <code>${esc(r.details.backup)}</code><br>Midlertidig kopi: <code>${esc(r.details.temp)}</code></p>` : ''}</div></div><div class="actions return-row-actions">${r.state === 'ready' ? `<button class="button small primary" data-action="move" data-id="${esc(r.id)}">Flyt</button>` : ''}${r.state === 'ambiguous' ? `<label>Original<select data-choice="${esc(r.id)}"><option value="">Vælg rigtig original…</option>${r.details.candidates.map(file => `<option value="${esc(file)}">${esc(file)}</option>`).join('')}</select></label>` : ''}${r.state === 'failed' ? `<button class="button small secondary" data-action="retry" data-id="${esc(r.id)}">Prøv igen</button>` : ''}${r.state === 'attention' ? `<button class="button small secondary" data-action="restore" data-id="${esc(r.id)}">Gendan original</button>` : ''}</div></article>`).join('') || '<div class="empty"><h2>Ingen filer i denne visning</h2><p>Færdige resultater vises efter stabilitetskontrollen. Brug Scan nu, eller tilføj en fra- og tilmappe.</p></div>';
...
function confirm(action, ids) {
  ...
  $('confirm-submit').disabled = deleting; $('confirm-submit').className = 'button ' + (deleting ? 'danger' : 'primary');
...
document.addEventListener('change', async e => {
  const el = e.target;
  if (el.dataset.select) { const set = el.dataset.select === 'old' ? oldSelected : selected; if (el.checked && set.size >= 100) { el.checked = false; toast('Vælg højst 100 filer.'); } else if (el.checked) set.add(el.value); else set.delete(el.value); syncSelection(); }
  if (el.dataset.choice && el.value) { el.blur(); await action('/' + el.dataset.choice + '/choose','POST',{original:el.value}); }
});
$('select-page').onchange = () => { const rows = visible.filter(r => r.state === 'ready'); if ($('select-page').checked && new Set([...selected,...rows.map(r=>r.id)]).size > 100) toast('Vælg højst 100 filer.'); else for (const r of rows) if ($('select-page').checked) selected.add(r.id); else selected.delete(r.id); syncSelection(); };
...
$('confirm-form').onsubmit = async e => {
  ...
  const ok = await action(url,'POST',{ids:p.ids,confirmation:p.action === 'delete' ? 'SLET OLD' : undefined});
  if (ok) { toast(p.action === 'move' ? 'Filerne er sat i flyttekø.' : p.action === 'delete' ? 'De valgte OLD-filer er slettet.' : 'Originalen er gendannet.'); selected.clear(); oldSelected.clear(); syncSelection(); }
};
