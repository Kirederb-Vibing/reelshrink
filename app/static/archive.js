...
function placeLabels(){const type=$('place-type').value,smb=type==='smb',sftp=type==='sftp';$('place-user-label').firstChild.textContent=type==='s3'?'Access key':'Bruger';$('place-user').placeholder=type==='s3'?'AKIA...':'brugernavn';$('place-share-label').hidden=sftp;$('place-share').required=!sftp;$('place-share-label').firstChild.textContent=smb?'Share':'Bucket';$('place-share').placeholder=smb?'movies':'bucket';$('place-path-label').firstChild.textContent=sftp?'Sti på serveren':'Mappe inde i sharen (valgfri)';$('place-path').placeholder=sftp?'/srv/storage/media/movies':'Film';$('place-path-hint').textContent=sftp?'Skriv den fulde sti med skråstreg foran, fx /srv/storage/media/movies. Uden skråstreg søges der i brugerens hjemmemappe.':'Share er navnet, du ser i filhåndteringen, for eksempel movies. Skriv ikke NAS\'ens interne sti.';}
$('place-type').onchange=placeLabels;placeLabels();
$('place-form').onsubmit=e=>{e.preventDefault();action(async()=>{
  const type=$('place-type').value,host=$('place-host').value.trim(),user=$('place-user').value.trim(),share=$('place-share').value.trim().replace(/^\/+|\/+$/g,''),rawPath=$('place-path').value.trim(),sub=(type==='sftp'&&rawPath.startsWith('/')?'/':'')+rawPath.replace(/^\/+|\/+$/g,'');
