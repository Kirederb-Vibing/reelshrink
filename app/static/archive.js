...
function workItems(){const q=$('work-search').value.toLowerCase(),filter=$('work-filter').value;return status.items.filter(r=>(filter==='all'||filter==='ready'&&r.state==='ready'||filter==='pending'&&!['sent','cleaned'].includes(r.state))&&(r.source+' '+r.local_source).toLowerCase().includes(q));}
...
$('upload').onclick=()=>{sending=status.items.filter(r=>uploads.has(r.id)&&r.canSend).map(r=>r.id);$('send-description').textContent='De valgte kontrollerede resultater sendes tilbage. Originalvideoen erstattes efter checksumkontrol; tilhørende filer i destinationsmappen bevares.';$('send-paths').textContent=status.items.filter(r=>sending.includes(r.id)).map(r=>r.source).join('\n');$('send-confirmation-label').textContent='Skriv SEND OG ERSTAT';$('send-confirmation').value='';$('send-dialog').showModal();};
